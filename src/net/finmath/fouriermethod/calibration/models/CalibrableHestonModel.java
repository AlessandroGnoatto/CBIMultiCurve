...
import net.finmath.fouriermethod.calibration.*;
...
/**
 * This class creates new instances of a HestonModel and communicates with the optimization algorithm.
 * 
...
public class CalibrableHestonModel implements  CalibrableProcessInterface {
	 ...
	 * This implies that the user could create Heston models which are not admissible in the sense of Duffie Filipovic and Schachermayer (2003).
	...
	public CharacteristicFunctionInterface apply(double time) {
		return (this.getCharacteristiFunction()).apply(time);
	}
	...
	private double[] extractLowerBounds() {
		...
		double threshold = -1E6;
		lowerBounds[0] = volatilityInfo.getConstraint().getLowerBound() < threshold ? threshold : volatilityInfo.getConstraint().getLowerBound();
		lowerBounds[1] = thetaInfo.getConstraint().getLowerBound() < threshold ? threshold : thetaInfo.getConstraint().getLowerBound();
		lowerBounds[2] = kappaInfo.getConstraint().getLowerBound() < threshold ? threshold : kappaInfo.getConstraint().getLowerBound();
		lowerBounds[3] = xiInfo.getConstraint().getLowerBound() < threshold ? threshold : xiInfo.getConstraint().getLowerBound();
		lowerBounds[4] = rhoInfo.getConstraint().getLowerBound() < threshold ? threshold : rhoInfo.getConstraint().getLowerBound();
