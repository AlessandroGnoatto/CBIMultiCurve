...
public class CapletCalibrationTestCorrectCurves {
	public static void main(String[] args) throws SolverException, CloneNotSupportedException {
		 ...
		 */
		double b = 0.012853146094575475;
		double sigma = 1.3401925431910914E-5;
		double eta =  0.0019340314648828437;
		double zeta = 1.0275347934025882;
		double alpha = 1.6008870570742983;
		double[] initialValues = {6.900135555153209E-5, 1.0018383269013605E-4};
		double[] immigrationRates = {5.074241939497439E-4, 0.002552433329055376};
		double[] lambda = {1, 1};
		...
		System.out.println("Errors in volatilities");
		for(String myString : errorsOverview)
			System.out.println(myString);
		
		
		System.out.println("Errors in prices.");
		QuotingConvention priceConvention = QuotingConvention.PRICE;
		MultivariateCalibrableProcessInterface myCBIcalibratedModel = result.getModel();
		
		for(int index = 0; index <indeces.length; index++) {
			double[] currentStrikes = surface.getSmile(maturities[index]).getStrikes();
			String underlying = surface.getSmile(maturities[index]).getUnderlyingCurve();
			
			EuropeanOptionSmileMultiAsset newPricer = pricer.getCloneWithModifiedParameters(underlying,maturities[index],currentStrikes);
			
			try {
				Map<Double, Double> currentModelPrices = newPricer.getValue(myCBIcalibratedModel);

				for(int i = 0; i<currentStrikes.length;i++) {
						double modelPrice = currentModelPrices.get(currentStrikes[i]);
						double marketPrice = surface.getValue(maturities[index], currentStrikes[i], priceConvention);
						System.out.println(currentStrikes[i] + "\t" + maturities[index] + "\t" + modelPrice + "\t"+  marketPrice);
				}
